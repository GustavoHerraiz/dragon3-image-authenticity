...
export class GeneradorMBH {
   ...
   async sellarImagen(rutaEntrada, rutaSalida, metadatosCliente) {
    try {
        ...
        try {
            // --- 💉 1. PERSISTENCIA INTEGRAL (RAM + DISCO) ---
            const nuevaEntrada = {
            ...
            const existe = BASE_DE_DATOS_SELLOS.some(s => s.hash_suffix === selloHex);
            if (!existe) {
                BASE_DE_DATOS_SELLOS.push(nuevaEntrada);
                try {
                    const rutaBD = './base_datos_sellos.js';
                    let contenido = fs.readFileSync(rutaBD, 'utf8');
                    const inyeccion = `,\n    {\n        hash_suffix: "${selloHex}",\n        cliente: "${nuevaEntrada.cliente}",\n        obra: "${nuevaEntrada.obra}"\n    }\n];`;
                    const nuevoContenido = contenido.replace(/\s*\]\s*;\s*$/, inyeccion);
                    fs.writeFileSync(rutaBD, nuevoContenido, 'utf8');
                    console.log(`[DB FÍSICA] Sello ${selloHex} persistido en disco.`);
                } catch (err) {
                    console.error("⚠️ Error escribiendo en base_datos_sellos.js:", err);
                }
            }
            ...
            try {
                ...
                const cmd = `exiftool -TagsFromFile "${rutaEntrada}" "-exif:all<exif:all" "-xmp:all<xmp:all" -ImageDescription="DRAGON3_ID:${selloHex}" -Copyright="${metadatosCliente.obra || 'Dragon3 Protected'}" -Artist="${metadatosCliente.cliente || 'Blade Corporation'}" -Software="Dragon3 V22 Mentalist Core" -overwrite_original "${rutaSalida}"`;
