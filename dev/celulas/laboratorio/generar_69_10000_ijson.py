...
def generar_dataset():
    print("=" * 80)
    print("GENERACIÓN DE 10000 MUESTRAS (69 features) CON ijson")
    print("=" * 80)
    ...
    if len(humanos) < TARGET or len(ias) < TARGET:
        print("❌ No hay suficientes muestras. Se necesitan 5000 de cada clase.")
        sys.exit(1)
    ...
    if len(X_list) < 10000:
        print(f"❌ Solo se obtuvieron {len(X_list)} muestras (<10000).")
        sys.exit(1)
    ...
    np.save(os.path.join(LAB_DIR, "X_69_10000.npy"), X)
    np.save(os.path.join(LAB_DIR, "y_69_10000.npy"), y)

    print(f"\n💾 Archivos guardados en:")
    print(f"   {os.path.join(LAB_DIR, 'X_69_10000.npy')}")
    print(f"   {os.path.join(LAB_DIR, 'y_69_10000.npy')}")

    print("\n✅ Ahora ejecuta: python analisis_69_10000.py")
