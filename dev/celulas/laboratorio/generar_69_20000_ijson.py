...
def generar_dataset():
    print("=" * 80)
    print("GENERACIÓN DE 20000 MUESTRAS (69 features) CON ijson")
    print("=" * 80)

    humanos = []
    ias = []
    TARGET = 10000  # Necesitamos 10000 de cada clase
    ...
    if len(humanos) < TARGET or len(ias) < TARGET:
        print("❌ No hay suficientes muestras. Se necesitan 10000 de cada clase.")
        sys.exit(1)
    ...
    if len(X_list) < 20000:
        print(f"❌ Solo se obtuvieron {len(X_list)} muestras (<20000).")
        sys.exit(1)
    ...
    np.save(os.path.join(LAB_DIR, "X_69_20000.npy"), X)
    np.save(os.path.join(LAB_DIR, "y_69_20000.npy"), y)

    print("\n💾 Archivos guardados en:")
    print(f"   {os.path.join(LAB_DIR, 'X_69_20000.npy')}")
    print(f"   {os.path.join(LAB_DIR, 'y_69_20000.npy')}")
    print("\n✅ Ahora ejecuta: python analisis_xgboost_20000.py")
