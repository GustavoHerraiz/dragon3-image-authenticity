...
def generar_dataset():
    print("=" * 80)
    print("GENERACIÓN DE 30000 MUESTRAS (69 features)")
    print("=" * 80)
        ...
        with open(json_file, 'rb') as f:
            for doc in ijson.items(f, 'item'):
                ...
                if "IA" in feedback:
                    ...
                    humanos.append(doc)
                # Log cada 500 documentos recolectados (como en el de 20000)
                total_actual = len(humanos) + len(ias)
                if total_actual % 500 == 0:
    ...
    if len(X_list) < 30000:
        print(f"❌ Solo se obtuvieron {len(X_list)} muestras (<30000).")
        sys.exit(1)
    ...
    np.save(os.path.join(LAB_DIR, "X_69_30000.npy"), X)
    np.save(os.path.join(LAB_DIR, "y_69_30000.npy"), y)

    print("\n💾 Archivos guardados en:")
    print(f"   {os.path.join(LAB_DIR, 'X_69_30000.npy')}")
    print(f"   {os.path.join(LAB_DIR, 'y_69_30000.npy')}")
    print("\n✅ Ahora ejecuta: python analisis_xgboost_30000.py")
