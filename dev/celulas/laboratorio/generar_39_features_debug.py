...
JSON_FILES = [
    ...
    # Añade los demás si quieres, pero primero probemos con uno
]
...
def generar_dataset():
    print("=" * 80)
    print("GENERACIÓN DE DATASET DE 39 FEATURES (200 muestras) con DEPURACIÓN")
    print("=" * 80)
        ...
        try:
            with open(json_file, 'rb') as f:
                for doc in ijson.items(f, 'item'):
                    total_processed += 1
                    # Depuración: si no es dict, lo mostramos
                    if not isinstance(doc, dict):
                        print(f"\n⚠️ Elemento #{total_processed} NO es dict, es {type(doc)}. Contenido (primeros 200 chars): {repr(str(doc)[:200])}")
                        errors += 1
                        continue
                    ...
                    total = len(X_list)
                    print(f"\r  Muestras recolectadas: {total} / 200  (Procesados: {total_processed}, Errores: {errors})", end='')
                    if total >= 200:
                        print()
                        break
    ...
    print("\n" + "-" * 80)
    print(f"Total elementos procesados: {total_processed}")
    print(f"Errores (no dict): {errors}")
    print(f"Muestras recolectadas: {len(X_list)}")

    if len(X_list) < 200:
        print(f"\n❌ No se alcanzaron las 200 muestras.")
        sys.exit(1)
