...
def generar_dataset():
    print("=" * 80)
    print("GENERACIÓN DE 5000 MUESTRAS (69 features) CON ijson")
    print("=" * 80)

    humanos = []
    ias = []
    TARGET = 2500  # Necesitamos 2500 de cada clase
    ...
    if len(humanos) < TARGET or len(ias) < TARGET:
        print("❌ No hay suficientes muestras. Se necesitan 2500 de cada clase.")
        sys.exit(1)
    ...
    if len(X_list) < 5000:
        print(f"❌ Solo se obtuvieron {len(X_list)} muestras (<5000).")
        sys.exit(1)
    ...
    X_path = os.path.join(LAB_DIR, "X_69_5000.npy")
    y_path = os.path.join(LAB_DIR, "y_69_5000.npy")
    np.save(X_path, X)
    np.save(y_path, y)
    ...
    print("\n✅ Ahora ejecuta: python analisis_69_5000.py")
