...
// --- IMPORTS V17 (DUAL ENGINE) ---
import { GeneradorMBH_v17 } from './generadorMBH_v17.js';
import { analizadorImagenMBH_v17 as analizadorV17 } from './analizadorMBH_v17.js';
...
const MASTER = path.join(DIRECTORIO_SALIDA, 'Atardecer_V17_Dual_Master.png'); // Nombre actualizado
const TILE_SIZE = 256;
...
// ============================================================================
// EJECUCIÓN DEL TEST BRUTAL
// ============================================================================

async function ejecutarTestBrutal() {
    console.log('\n============================================================');
    console.log('🐉 DRAGON3 - TEST V21 "FAÚNDEZ TORTURE CHAMBER" (ENGINE V17)');
    console.log('============================================================\n');
    ...
    // --- FASE 1: GENERACIÓN V17 (DUAL) ---
    console.log('[FASE 1] 🧬 Generando Master V17 (Vogel + Stardust DCT)...');
    const inicioGen = Date.now();

    // INSTANCIAMOS EL GENERADOR V17
    const gen = new GeneradorMBH_v17();

    // Inyectamos un ID conocido para que coincida con la DB simulada del Analizador
    ...
    // --- FASE 2: CONTROL DE CALIDAD (NOTARÍA) ---
    console.log('\n[FASE 2] 🔍 Verificando integridad del Master (Modo Revista)...');

    // USAMOS EL ANALIZADOR V17
    const control = await analizadorV17(MASTER);

    const evMaster = control && control.response ? control.response.evidencia_visual : null;

    if (!evMaster || !control.response.exito) {
        console.error("❌ ERROR CRÍTICO: El Master no ha pasado la certificación Vogel.");
        console.log(evMaster);
        throw new Error("Abortando test por fallo en Master.");
    }

    console.log(`   ✅ MASTER OK: ${evMaster.Metodo} | Confianza: ${evMaster.Mejor_Confianza}%`);

    // --- FASE 3: CÁMARA DE TORTURA (DETECTIVE) ---
    console.log('\n[FASE 3] ⚔️ Ejecutando Batería de Tortura (Modo Forense)...\n');
    const resultados = [];
    ...
    for (const esc of ESCENARIOS) {
        ...
        // Analizar con V17 (debe saltar al modo Stardust DCT)
        const r = await analizadorV17(rutaS);

        const ev = r && r.response ? r.response.evidencia_visual : null;

        // Criterio de éxito: El analizador dice que SÍ
        const exito = r && r.response && r.response.exito;
        ...
        resultados.push({
            ...
            hits: ev ? ev.Hits_Totales : 0,
            confianzaPct: ev ? ev.Mejor_Confianza : 0,
            metodo: ev ? ev.Metodo : "Ninguno",
    ...
    const jsonData = {
        metadata: { version: '17.0 Dual', master: evMaster },
    ...
    try {
        ...
        // (Aquí llamarías al resto de visuales si los has copiado)
    } catch (e) { console.log("   ⚠️ Visuales saltados o error parcial."); }

    const md = generarInformeMarkdown(jsonData, resultados, stats, {}, duracionGen);
    fs.writeFileSync(path.join(DIRECTORIO_SALIDA, `INFORME_V17_DUAL.md`), md);

    // --- FASE 5: TABLA FINAL ---
    console.log('\n============================================================');
    console.log('📊 RESUMEN DE SUPERVIVENCIA NANO-HYDRA V17 (DUAL ENGINE)');
