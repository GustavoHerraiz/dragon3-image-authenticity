...
// ================================================================
// 🔧 FUNCIÓN PARA ENCONTRAR EXIFTOOL
// ================================================================
...
// ==========================================================
try {
    // ✅ SIEMPRE usar proyectoId si existe
    if (metadatosCliente.proyectoId) {
        const proyectoExistente = await this.db.buscarPorId(metadatosCliente.proyectoId);
        if (proyectoExistente) {
            await this.db.registrarSello(
                idNumerico,
                idHex,
                metadatosCliente.proyectoId,
                metadatosCliente.cliente,
                metadatosCliente.obra,
                metadatosCliente.coleccion,
                metadatosCliente.derechos,
                metadatosCliente.email_contacto,
                metadatosCliente.compartir_blade || 0
            );
            telemetry.info(MODULE, `✅ Sello ${idCompleto} registrado en proyecto ${metadatosCliente.proyectoId}`);
        } else {
            ...
            const nuevoProyecto = await this.db.crearProyecto(
                ...
                metadatosCliente.obra || 'Obra automática',
                metadatosCliente.proyecto_nombre || `Proyecto_${idNumerico}`,
            ...
            telemetry.info(MODULE, `✅ Proyecto creado y sello registrado: ${nuevoProyecto.id}`);
        }
    } else {
...
// ==========================================================
// 14. INYECCIÓN DE METADATOS (TÉCNICA DEL SÁNDWICH)
// ==========================================================
try {
    const exifPath = getExifToolPath();
    if (!exifPath) {
        telemetry.warn(MODULE, `⚠️ ExifTool no encontrado.`);
    } else {
        // ============================================================
    ...
    // =========================================================================
// 🛰️ MOTOR GEOMÉTRICO (Vogel) - CORREGIDO
// =========================================================================
